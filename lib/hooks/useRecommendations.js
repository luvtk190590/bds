...
export function useRecommendations(profileId, limit = 10) {
    const { data, error, isLoading, mutate } = useSWR(
        ...
        async () => {
            ...
            // Nếu hàm chưa tồn tại hoặc lỗi → trả mảng rỗng, không crash
            if (error) return [];
            return data || [];
        },
        {
            ...
            onError: () => {},  // suppress SWR error boundary
        }
    );

    return { recommendations: data || [], isLoading, error: null, mutate };
}
...
export function useUserPreferences(profileId) {
    ...
    return { preferences: data, isLoading, error: null };
}
...
export async function trackPropertyView(
    ...
    const supabase = createClient();
    // Bỏ qua lỗi nếu hàm chưa tồn tại
