...
export default function DetailsTitle1({ propertyItem = {}, onFavoriteToggle, isFavorited = false }) {
  const address = propertyItem?.address || [
    propertyItem?.ward?.name,
    propertyItem?.district?.name,
    propertyItem?.province?.name,
  ].filter(Boolean).join(", ");
        ...
        <div className="header-property-detail">
          <div className="content-top d-flex justify-content-between align-items-center">
            ...
            <div className="box-price d-flex align-items-end">
              <h3 className="fw-8">
                {propertyItem?.price ? formatPriceWithUnit(propertyItem.price, propertyItem.listing_type) : "Đang cập nhật"}
              </h3>
            </div>
          </div>
          <div className="content-bottom">
            <div className="box-left">
              <div className="info-box">
                <div className="label">Đặc điểm</div>
                <ul className="meta">
                  <li className="meta-item">
                    <i className="icon icon-bed" />
                    <span className="text-variant-1">Phòng ngủ:</span>
                    <span className="fw-6">{propertyItem?.bedrooms || "--"}</span>
                  </li>
                  <li className="meta-item">
                    <i className="icon icon-bath" />
                    <span className="text-variant-1">Phòng tắm:</span>
                    <span className="fw-6">{propertyItem?.bathrooms || "--"}</span>
                  </li>
                  <li className="meta-item">
                    <i className="icon icon-sqft" />
                    <span className="text-variant-1">Diện tích:</span>
                    <span className="fw-6">{propertyItem?.area ? formatArea(propertyItem.area) : "--"}</span>
                  </li>
                </ul>
              </div>
              <div className="info-box">
            ...
            <ul className="icon-box">
              ...
              <li>
                <button
                  onClick={() => onFavoriteToggle && onFavoriteToggle(propertyItem?.id)}
                  className={`item fav-btn${isFavorited ? " is-favorited" : ""}`}
