-- Migration 014: Replace filter_type int with filter_type_name text, add min_bathrooms

-- Drop old versions
DROP FUNCTION IF EXISTS public.properties_in_view(float, float, float, float, int, text, numeric, numeric, numeric, numeric, int, text);
DROP FUNCTION IF EXISTS public.properties_in_view(float, float, float, float, int, text, numeric, numeric, numeric, numeric, int);
