-- Migration 043: Fix recommendation functions (drop + recreate để sửa return type)

DROP FUNCTION IF EXISTS public.get_user_preference_profile(uuid);
DROP FUNCTION IF EXISTS public.get_recommendations(uuid, int);
DROP FUNCTION IF EXISTS public.track_property_view(uuid, uuid, int);
